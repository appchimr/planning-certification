 ...
 *
 * Le Google Sheet reste privé sur le compte Communication CHIMR.
 * La lecture du planning est publique via l'application web.
 * Les écritures nécessitent une clé d'administration stockée
 * dans les Propriétés du script sous le nom ADMIN_KEY.
 */

const SPREADSHEET_ID = "10nbKtQ2xISLU0riOfVgslWwBh10YTSZaWZhFWVw2NiA";
const SHEET_NAME = "Planning";
const HEADERS = ["ID", "Mois", "Type", "Libellé", "Réalisé", "Ordre", "Modifié le"];
...
function doPost(e) {
  ...
  try {
    ...
    if (action === "auth") {
      return jsonResponse_({ ok: isAdminKeyValid_(payload.adminKey) });
    }

    if (!isAdminKeyValid_(payload.adminKey)) {
      return jsonResponse_({ ok: false, error: "Clé d’administration invalide." });
    }

    if (action === "saveAll") {
      if (!Array.isArray(payload.events)) {
        return jsonResponse_({ ok: false, error: "Liste d’événements invalide." });
      }
...
function isAdminKeyValid_(candidate) {
  ...
  if (!expected) {
    throw new Error("La propriété de script ADMIN_KEY n’est pas configurée.");
  }
